...
def train(args, data_info):
    ...
    user_interact_item_acc = []  # 用户交互频率集
    i = 0
    while i < n_user:
        user_interact_item_acc.append(len(user_interacted_item[i]))
        i = i + 1

    item_interacted_user = []  # 项目历史用户集
    ...
    item_interact_user_acc = []  # 项目交互频率集
    i = 0
    while i < n_item:
        item_interact_user_acc.append(len(item_interacted_user[i]))
        i = i + 1

    model, optimizer, loss_func = _init_model(args, data_info, item_interact_user_acc)
    for step in range(args.n_epoch):
        ...
        logging.info(ctr_info, step+1, eval_auc, eval_f1, test_auc, test_f1)
        if args.show_topk:
...
def _init_model(args, data_info, item_interact_user_acc):
    n_entity = data_info[3]
    n_relation = data_info[4]
    model = ERCFA(args, n_entity, n_relation, item_interact_user_acc)
    if args.use_cuda:
        model.cuda()
